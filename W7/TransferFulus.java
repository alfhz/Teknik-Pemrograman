...
public class TransferFulus {
    public static void main(String[] args) throws InterruptedException {
        ...
        Thread t1 = new Thread(() -> {
            synchronized (acc1) { // Blok ini mengunci objek acc1 untuk memastikan bahwa hanya satu thread yang
                                  // dapat mengaksesnya pada saat yang sama, sehingga mencegah race condition saat
                                  // kedua thread mencoba mengakses acc1 bersamaan.
                System.out.println("Thread t1 sedang mengakses acc1 untuk mentranfer fulus ke acc2");
                try {
                    Thread.sleep(100);
                } catch (Exception e) {
                } // Simulasi dengan memberikan jeda. Mengapa diperlukan Exception? Karena untuk
                  // menangani kemungkinan InterruptedException yang dapat terjadi saat thread
                  // sedang tidur.

                synchronized (acc2) { // Blok ini mengunci objek acc2 untuk memastikan bahwa hanya satu thread yang
                                      // dapat mengaksesnya pada saat yang sama, sehingga mencegah race condition saat
                                      // kedua thread mencoba mengakses acc2 bersamaan.
